...
class Dataset:
    ...
    def preprogress(self, no_attributes=False, random_Attributes_dim=None):
        ...
        if self.dataset_name == 'cora' or self.dataset_name == 'citeseer':
            ...
            G = nx.read_edgelist(edges_path, nodetype=int, create_using=nx.Graph())
            ...
            # 将边转换为坐标列表
            row, col = zip(*edges)
            data = np.ones(len(row))  # 假设所有边的权重都为1
            ...
            # # 使用节点度作为特征
            # nodes = np.arange(n_nodes)  # 假设节点是连续编号的
            ...
            # self.adj = adj
            # self.features = features
            # self.labels = torch.zeros(len(nodes), dtype=torch.long)  # 或根据需要设置标签

            # self.save()

            # return adj, features, self.labels

            # 将边转换为坐标列表

            feature_dim = 128  # 设置特征维度，例如 128
            features = torch.rand(n_nodes, feature_dim)  # 生成随机特征
            ...
            # print("adj shape after coo_matrix: ***", adj.shape)
            
            # 这个数据集没有提供标签信息，您需要根据实际情况调整
            labels = None
            ...
            # print("dimension feature ***: ", features.shape)
            ...
            # feature_dim = 128  # 设置特征维度，例如 128
            feature_file_path = self.work_path + '/data/' + self.dataset_name + '/features.pt'
            if os.path.isfile(feature_file_path):
                ...
                # 否则，生成随机特征并保存
                feature_dim = 128  # 设置特征维度
                features = torch.rand(n_nodes, feature_dim)  # 生成随机特征
                self.save_features(features, feature_file_path)
                print("Save Features.")
            ...
            # Assuming 'edges' is an array of shape (num_edges, 2)
            all_nodes = np.unique(edges.flatten())
            n_nodes = len(all_nodes)
            node_mapping = {old_id: new_id for new_id, old_id in enumerate(all_nodes)}

            row = np.array([node_mapping[id] for id in edges[:, 0]])
            col = np.array([node_mapping[id] for id in edges[:, 1]])
